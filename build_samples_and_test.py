from utils.dataset import GapFillDataset
from utils.encoding import load_fasta  # you’ll need a helper like this

# 1) Load full genome string
genome = load_fasta("data/raw/GCA_000027325.1_ASM2732v1_genomic.fna")

# 2) Build a small list of (left, right, gap) triples
def make_samples(genome, flank_len=200, gap_len=50, n_samples=1000):
    samples = []
    import random
    max_start = len(genome) - (2*flank_len + gap_len)
    for _ in range(n_samples):
        s = random.randint(0, max_start)
        left = genome[s : s + flank_len]
        gap = genome[s + flank_len : s + flank_len + gap_len]
        right = genome[s + flank_len + gap_len : s + 2*flank_len + gap_len]
        samples.append((left, right, gap))
    return samples

samples = make_samples(genome, flank_len=200, gap_len=50, n_samples=1000)

dataset = GapFillDataset(samples)

left_flank, right_flank, gap = dataset[0]
print("Left flank shape:", left_flank.shape)
print("Right flank shape:", right_flank.shape)
print("Gap shape:", gap.shape)
